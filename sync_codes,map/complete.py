...
def summ():
	full_dict = {"skin":{"Scabies":"sc",	"Pityriasis":"pi",	"Phrynoderm":"ph",	"Pediculosi":"pe",	"Pityriasis":"pity",	"Impetigo":"im",	"Papularurt":"pap",	"Tinea Crus":"tc",	"Miliaria":"mi",	"Viral Wart":"vi",	"Xerosis":"xerosis",	"Referral":"referal"},
				...
				"health1":{	"Frequency of skipping Breakfast":"frequencyFood","Over Night Food":"over_night_food","Both Parents Working":"both_parents",	"Untrimmed Nails":"ph_n",	"Irregular Bathing":"ph_b",	"Poor Grooming":"ph_g",	"Bad Oral Care":"ph_oc",	"Age of Menarche":"ph_am",	"Sanitary Napkin":"ph_am_sn",	"Clinical Anemia":"ca",	"Dental Care":"oe_dc",	"Fluorosis":"oe_f",	"Gingivitis":"oe_g",	"Oral Ulcer":"oe_ou",	"Trauma":"oe_trauma",	"Spacing":"oe_spacing",	"Crowding":"oe_crowding","Oral Referal":"oe_referal",	"Chronic Co":"rs_chronic",	"Brochial Asthma":"rs_bronchial",	"Adventitous sounds":"rs_sounds",	"Abnormal Heart Sounds":"cvs_ahs"},
...
def ref():
	...
	curr.execute("SELECT distinct(name),school_id from school")
	row = curr.fetchall()
	for r in row:
		fo.write(r[0]+"\n")
		fo.write("----------------------------------------------------\n")
		curr.execute("SELECT DISTINCT(child_id),name from child where school_id = %(1)s"%{"1":r[1]})
		chd = curr.fetchall()
		for k in chd:
			for i in ref_ls:
				if i == "health1":
